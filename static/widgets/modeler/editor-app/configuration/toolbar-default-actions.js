...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
  function ($rootScope, $scope, $http, $route, $location) {
    ...
    var tenantId = EDITOR.UTIL.getParameterByName('tenantId')
    var saveDialog = {
    ...
    $scope.close = function () {
    	// czj:取消时不关闭设计器（注释以下3行）
//    	if(parent.window&&parent.window.closeDesignWin){
//			parent.window.closeDesignWin();
//		}
    ...
    $scope.save = function (successCallback) {
      ...
      var json = $scope.editor.getJSON()
      var numReg = /^([\d])/// 数字开头
      if (!json.properties.process_id || json.properties.process_id.length == 0 || numReg.test(json.properties.process_id)) {
        	alert('流程标识不能为空且不能数字开头')
        return false
      }
