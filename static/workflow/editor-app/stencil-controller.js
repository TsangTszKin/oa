      ...
      $scope.propertyClicked = function (index) {
        ...
        if ($scope.selectedItem.properties[index].key == 'oryx-formkeydefinition') { // 表单key值
          ...
          function receiveMessageFromIndex (event) {
            ...
            $scope.selectedItem.properties[index].value = event.data
            // $scope.selectedItem.properties[index].mode = 'write'
            $scope.$apply()
            console.log('$scope.selectedItem.properties[index].value', $scope.selectedItem.properties[index].value)
          }

          // 监听message事件
          window.addEventListener('message', receiveMessageFromIndex, false)
          parent.postMessage(JSON.stringify({ code: 0, value: $scope.selectedItem.properties[index].value }), '*') // window.postMessage
        }
      }
