...
angular.module('activitiModeler')
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      /*
       StencilSet items
       */
      $http({ method: 'GET', headers: { 'Authorization': localStorage.token }, url: KISBPM.URL.getStencilSet() }).success(function (data, status, headers, config) {
    ...
    $scope.propertyClicked = function (index) {
      console.log('$scope.selectedItem.properties[index] propertyClicked', $scope.selectedItem.properties[index])
      if (!$scope.selectedItem.properties[index].hidden) {
        $scope.selectedItem.properties[index].mode = 'write'
      }
      if ($scope.selectedItem.properties[index].key == 'oryx-formkeydefinition') { // 表单key值
        $scope.selectedItem.properties[index].mode = 'read'
        // 回调函数
        function receiveMessageFromIndex (event) {
          console.log('我是iframe,我接受到了：', event.data)

          $scope.selectedItem.properties[index].value = JSON.stringify(event.data)
          $scope.selectedItem.properties[index].noValue = false
          $scope.updatePropertyInModel($scope.selectedItem.properties[index])
          $scope.$apply()
          console.log('$scope.selectedItem.properties[index]', $scope.selectedItem.properties[index])
        ...
        try {
          parent.postMessage({ code: 0, value: JSON.parse($scope.selectedItem.properties[index].value).value }, '*') // window.postMessage
        } catch (error) {
          parent.postMessage({ code: 0, value: $scope.selectedItem.properties[index].value }, '*') // window.postMessage
        }
      }
    }
    ...
    $scope.updatePropertyInModel = function (property, shapeId) {
      console.log('property, shapeId', property, shapeId)
  ...
  }]).filter('nameFormat', function () { // 可以注入依赖
    return function (value) {
      console.log('value=', value)
      if (value && typeof value === 'string' && value.indexOf('{') != -1 && value.indexOf('}') != -1 && value.indexOf('label') != -1) {
        try {
      ...
      if (value && typeof value === 'object' && value.assignment && value.assignment.candidateUsers && value.assignment.candidateUsers instanceof Array) {
        let tempArray = []
        value.assignment.candidateUsers.forEach(function (element) {
          tempArray.push(element.label)
        })
  ...
  }).filter('shouliren', function () { // 可以注入依赖
    return function (value) {
      console.log('value', value)
