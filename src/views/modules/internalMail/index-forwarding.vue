  ...
  <div>
    <el-dialog
      ...
      <div v-loading="!submitAble" :style="{paddingBottom: footerShow ? '' : '50px'}">
        <sende-mail ref="sendeMail" :ueId="'forwarding'" :changeMail="true" @refreshDataList="refreshDataList" @formDataNopass="formDataNopass" @write-a-letter="writeALetter"></sende-mail>
      </div>
      <span slot="footer" class="dialog-footer" v-if="footerShow">
        ...
        <el-button @click="visible = false; $emit('closeForwarding'); $refs.sendeMail.clearAutosave()" :loading="!submitAble" size="mini">关闭</el-button>
      </span>
    </el-dialog>
    <!-- 弹窗, 选择企业 -->
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          this.$refs.sendeMail.replyForwardingInit(this.dataForm.id, '转发：')
        })
      },
